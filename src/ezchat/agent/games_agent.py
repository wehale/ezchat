...
Run with:
    ezchat --agent games --server http://SERVER:8000 --handle games
"""
...
def _games_list() -> str:
    from ezchat.games import list_games
    games = list_games()
    if not games:
...
async def run_games_agent(identity, server: str) -> None:
    ...
    from ezchat.net.connection import accept_peer
    from ezchat.games import SessionRouter
    ...
    async def _handle_message(sender: str, text: str) -> None:
        ...
        from ezchat.games import get_game_class
